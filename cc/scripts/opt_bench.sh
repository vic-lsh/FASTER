...
for z in 0.7 0.9 0.99; do
	for dd in {5..28}; do
		d=$((2 * $dd))  # DRAM size
		FILE="result_opt_z_${z}_v_${v}_t_${t}_d_${d}_n_${n}.txt"
		PERF_FILE="result_perf_opt_z_${z}_v_${v}_t_${t}_d_${d}_n_${n}.txt"

		# Start benchmark
		rm -rf $FILE
		touch $FILE
		numactl --cpunodebind 0 ./pmem_benchmark 2 8 $t $z $n $num_op $num_warmup_op $d > ${FILE}&
