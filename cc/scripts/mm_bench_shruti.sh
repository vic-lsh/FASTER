...
do
  echo ------RUN $value---------
	...
	rm -rf $FILE
	touch $FILE
	numactl --cpunodebind 0 --membind 0 colorctl 0-$(($d * 8 - 1)) 0 ./pmem_benchmark 2 8 $t 0.9 $n $num_op $num_warmup_op > ${FILE}&
  ...
  sudo /root/yuhong/linux/tools/perf/perf script -F addr,phys_addr,time,event > z_10g_$value.txt
done
